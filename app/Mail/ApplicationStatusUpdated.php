...
class ApplicationStatusUpdated extends Mailable
{
    ...
    {
        $subject = 'Your Election Application Status Update';

        if ($this->application->status === 'approved') {
            $subject = 'Congratulations! Your Election Application Has Been Approved';
        } elseif ($this->application->status === 'rejected') {
            $subject = 'Update: Your Election Application Status';
        }

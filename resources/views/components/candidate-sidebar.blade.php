<!-- resources/views/components/candidate-sidebar.blade.php -->
{{-- This component displays the candidate's sidebar navigation. --}}
{{-- It receives the 'isSidebarOpen' state from its parent layout to control visibility of text. --}}

{{-- The main container for the sidebar. 'flex flex-col' makes it a vertical flex container. --}}
<div class="h-full py-4 px-3 bg-gray-800 text-gray-200 flex flex-col"> {{-- Added flex flex-col here --}}
    {{-- Top section with "Candidate Panel" / "CP" text --}}
    <div class="mb-6 flex items-center justify-between">
        {{-- "Candidate Panel" is shown when the sidebar is open --}}
        <div x-show="isSidebarOpen" class="text-xl font-semibold">{{ __('Candidate Panel') }}</div>
        {{-- "CP" is shown when the sidebar is collapsed --}}
    ...
    <div class="flex-grow">
        <ul class="space-y-2">
            ...
            <li>
                <a href="{{ route('profile.edit') }}" class="flex items-center p-2 text-base font-normal text-gray-300 rounded-lg hover:bg-gray-700 group"> {{-- Reverted to profile.edit --}}
